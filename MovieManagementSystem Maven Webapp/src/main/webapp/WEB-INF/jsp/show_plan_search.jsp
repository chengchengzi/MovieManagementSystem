...
<div id="container">
		...
		<table class="hovertable">
			...
			<c:forEach items="${listShowPlan}" var="listShowPlan" varStatus="status">
				<tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#d4e3e5';">
					...
					<td><fmt:formatDate type="both" value="${listShowPlan.show_time}"/></td>
					<td>
					<!-- 按钮 -->
    ...
    <div align="center" style="position: relative;top:10px;">  
        <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第  
            ${page.pageNow} 页</font> <a href="${pageContext.request.contextPath}/ShowPlan/searchShowPlan?pageNow=1">首页</a>  
        <c:choose>  
            <c:when test="${page.pageNow - 1 > 0}">  
                <a href="${pageContext.request.contextPath}/ShowPlan/searchShowPlan?pageNow=${page.pageNow - 1}">上一页</a>  
            </c:when>  
            <c:when test="${page.pageNow - 1 <= 0}">  
                <a href="${pageContext.request.contextPath}/ShowPlan/searchShowPlan?pageNow=1">上一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="${pageContext.request.contextPath}/ShowPlan/searchShowPlan?pageNow=${page.pageNow}">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
                <a href="${pageContext.request.contextPath}/ShowPlan/searchShowPlan?pageNow=${page.pageNow + 1}">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
                <a href="${pageContext.request.contextPath}/ShowPlan/searchShowPlan?pageNow=${page.totalPageCount}">下一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="${pageContext.request.contextPath}/ShowPlan/searchShowPlan?pageNow=${page.pageNow}">尾页</a>  
            </c:when>  
            <c:otherwise>  
                <a href="${pageContext.request.contextPath}/ShowPlan/searchShowPlan?pageNow=${page.totalPageCount}">尾页</a>  
            </c:otherwise>  
        </c:choose>  
    </div>  
    <!-- 分页功能 End -->	
	...
	//接受查询的影片编号
 	function showFilmIDMes(){
 	 	var show_plan_search_film_id = localStorage.getItem("show_plan_search_film_id");
 	 	document.getElementById("searchShowPlanFId").value = show_plan_search_film_id;
 	} 
...
<!-- 分页查询：放入查询参数 -->
<%
	String name = request.getParameter("searchShowPlanFId");
