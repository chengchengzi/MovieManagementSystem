...
<head>
	...
	%>  
	<link rel="stylesheet" type="text/css" href=".././css/ticket_seller_sale.css">
...
<div id="container">
	...
	<div id="all">
		<div id="sreachFilmHallById" style="position: relative;top:105px;left:220px">
			<form action="${pageContext.request.contextPath}/TicketSellerSaleInfo/searchSaleInfo">
		...
		<div id="mytable" align="center" style="position: relative;top:80px;">
			<table class="hovertable">
				...
				<c:forEach items="${listSaleInfo}" var="listSale" varStatus="status">
				<tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#d4e3e5';">
					...
					<td><fmt:formatDate type="both" value="${listSale.sale_info_time}"/></td>
    ...
    <div align="center" style="position: relative;top:10px;">  
        <font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第  
            ${page.pageNow} 页</font> <a href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo?pageNow=1">首页</a>  
        <c:choose>  
            <c:when test="${page.pageNow - 1 > 0}">  
                <a href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo?pageNow=${page.pageNow - 1}">上一页</a>  
            </c:when>  
            <c:when test="${page.pageNow - 1 <= 0}">  
                <a href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo?pageNow=1">上一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo?pageNow=${page.pageNow}">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 < page.totalPageCount}">  
                <a href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo?pageNow=${page.pageNow + 1}">下一页</a>  
            </c:when>  
            <c:when test="${page.pageNow + 1 >= page.totalPageCount}">  
                <a href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo?pageNow=${page.totalPageCount}">下一页</a>  
            </c:when>  
        </c:choose>  
        <c:choose>  
            <c:when test="${page.totalPageCount==0}">  
                <a href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo?pageNow=${page.pageNow}">尾页</a>  
            </c:when>  
            <c:otherwise>  
                <a href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo?pageNow=${page.totalPageCount}">尾页</a>  
            </c:otherwise>  
        </c:choose>  
    </div>  
    <!-- 分页功能 End -->	
...
<script type="text/javascript">
	...
	function delSaleInfoById(obj,id){
		...
		if(confirm("确认退票【" + content_sale_info_id   + "】号电影票么?")) {
			...
			$.ajax({
				type:"post",
				url: "${pageContext.request.contextPath}/TicketSellerSaleInfo/delSaleInfo",
				data:{'sale_info_id':content_sale_info_id},				
				success: function(res) {
					alert("退票成功");
					//回到本页
					window.location.href="${pageContext.request.contextPath}/TicketSellerSaleInfo/showSaleInfo";
				}
			});
		}
	}
    ...
    function checkAllDelete(){
		...
		if(sale_info_del_ids == ""){
    		...
    		$.ajax({
				type:"post",
				url: "${pageContext.request.contextPath}/TicketSellerSaleInfo/delBatchSaleInfo",
